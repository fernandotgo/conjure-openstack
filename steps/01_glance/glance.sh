...
do
    ...
    if [ ! -f $HOME/glance-images/bionic-server-cloudimg-${image_arch}-$imagetype ]; then
        debug "Downloading bionic image..."
        wget --user-agent="conjure-up/openstack-base" -qO ~/glance-images/bionic-server-cloudimg-${image_arch}-$imagetype https://cloud-images.ubuntu.com/bionic/current/bionic-server-cloudimg-${image_arch}-$imagetype
    fi
    ...
    bionic_image=bionic${imagesuffix}-${image_arch}
    if ! glance image-list --property-filter name="$bionic_image" | grep -q "$bionic_image" ; then
        debug "Importing $bionic_image"
        glance image-create --name="$bionic_image" \
               --container-format=bare \
               --disk-format=$diskformat \
               --property hw_firmware_type=$firmwaretype \
               --property architecture="${nova_arch}" \
               --visibility=public --file=$HOME/glance-images/bionic-server-cloudimg-${image_arch}-$imagetype > /dev/null 2>&1
    fi
done
